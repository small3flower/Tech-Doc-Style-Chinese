...
- 中文段落在源文件中保持一段一行，不为了控制行宽做硬换行；代码块、表格、标题和列表结构保持原样。
...
以下流程按任务模式选择执行；校对只执行约定范围内的检查和修改。
...
- 内容是否便于目标读者扫读和执行
- 段落是否残留硬换行，拼接边界的中西文留白是否正确
- 项目覆盖规则是否来自目标项目，而不是示例文件

## 参考资料路由

- 术语、黑话、错词、大小写、标点、中西文留白和段落换行：读取 [术语与排版](references/terminology-and-typography.md)
